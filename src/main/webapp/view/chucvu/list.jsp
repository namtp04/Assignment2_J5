...
<div class="container">
    <h1 class="mb-3 mt-5">Quản lý chức vụ</h1>
    <a class="btn btn-outline-success" href="/chuc-vu/view-add">Add</a>
    <div class="d-flex justify-content-end">
        <form action="/chuc-vu/search" method="post">
            <div class="input-group">
                <input name="chucVuSearchValue" type="text" class="form-control" placeholder="Enter name chuc vu"
                       aria-label="Search customer" value="${param.chucVuSearchValue}">
    ...
    <table class="table">
        ...
        <tbody>
        <c:forEach items="${lstCV}" var="cl">
            <tr>
                <td>${cl.ma}</td>
                <td>${cl.ten}</td>
                <td>
                    <a class="btn btn-outline-warning" href="/chuc-vu/view-update/${cl.id}">Update</a>
                    <a onclick="return confirm('Do you want to delete this item?')" class="btn btn-outline-danger"
                       href="/chuc-vu/delete/${cl.id}">Delete</a>
                </td>
            </tr>
        </c:forEach>
        <c:if test="${lstCV.size()==0}">
            <tr>
...
<script>
    window.onload = function() {
        ...
        alert('${deleteFail}');
        window.location.href='/chuc-vu/list';
        </c:if>
    };
</script>
</body>
